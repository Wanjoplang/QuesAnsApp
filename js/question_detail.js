document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleEditAnswer(answerIndex, answerData) {
        ...
        saveButton.addEventListener('click', () => saveEditedAnswer(answerIndex, answerData.id)); // Pass answer ID
        cancelButton.addEventListener('click', () => fetchQuestionDetails(questionId)); // Re-fetch to reset display
    }

    async function saveEditedAnswer(answerIndex, answerId) {
        const answerItem = answersListElement.children[answerIndex];
        if (!answerItem) return;
        ...
        try {
            // Assuming your API endpoint is PUT /questions/:questionId/answers/:answerId
            const response = await fetch(`${apiUrl}/questions/${questionId}/answers/${answerId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async function handleDeleteAnswer(answerIndex) {
        if (confirm('Are you sure you want to delete this answer?')) {
            try {
                // Assuming your API endpoint is DELETE /questions/:questionId/answers/:answerId
                const answerToDelete = currentQuestionData.answers[answerIndex];
                if (!answerToDelete?.id) {
                    console.error('Answer ID not found for deletion.');
                    alert('Could not delete the answer.');
                    return;
                }
                const response = await fetch(`${apiUrl}/questions/${questionId}/answers/${answerToDelete.id}`, {
                    method: 'DELETE',
                });
